...
public class AvgHandleTimeController {
    ...
    private static void checkAndAddAgentAHTInsight(String agentFirstName, Insights avgHandleTimeInsightList, String queueId, Double queueAvgHandleTimeValue, String queueName, String agentId, Double agentAvgHandleTimeValue) {
        if (agentAvgHandleTimeValue > queueAvgHandleTimeValue) {
            Insight insight = new Insight(InsightType.AVG_HANDLE_TIME_AGENT, agentAvgHandleTimeValue, "Agent " + agentFirstName + "’s average handling time is " + decfor.format(agentAvgHandleTimeValue / 60) + " minutes, which is much higher than the " + queueName + "’s average handling time of " + decfor.format(queueAvgHandleTimeValue / 60) + " minutes", "The reason for Agent " + agentFirstName + "'s high average handling time could be that she is not resolving customer issues efficiently. She may be taking too long to identify the customer's problem, not using available resources and tools effectively, or getting sidetracked during the call.", "To address this issue, I would recommend that you: 1. Provide additional training to Agent " + agentFirstName + " on call resolution techniques and time management. 2. Monitor her calls more closely and provide feedback on how to streamline her call-handling process.", new Metadata(queueId, agentId, List.of(BedrockSource.TRAINING_PT2, BedrockSource.TRAINING_PT3)));
